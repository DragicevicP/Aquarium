...
void Fish::init(float x_, float y_, float w_, float h_, float speed_, bool facingRight_) {
	x = x_; y = y_; w = w_; h = h_; speed = speed_; facingRight = facingRight_; texture = 0; nextBubbleTime = 0.0f;
}


void Fish::updateMovement(GLFWwindow* window, float deltaTime) {
	if (!window) return;
	// This function expects the caller to check appropriate keys for each fish.
	// We'll leave actual per-fish key mapping in main (since drawing is also in main).
}
...
void Fish::trySpawnBubbles(std::vector<Bubble>& bubbles, int& bubbleCount) {
	...
	if (bubbleCount > 0 && now >= nextBubbleTime) {
		...
		bubbles.push_back(b);
		bubbleCount--;
		nextBubbleTime = now + 0.3f; // delay between bubbles
	}
}